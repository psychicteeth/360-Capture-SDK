...
NVENCSTATUS CNvHWEncoder::NvEncOpenEncodeSession(void* device, uint32_t deviceType) {
  ...
  if (nvStatus != NV_ENC_SUCCESS) {
    DEBUG_ERROR_VAR("NVIDIA encoding driver assert in nvEncOpenEncodeSession. Don't open multiple encoding session at same time", to_string(nvStatus));
    assert(0);
  }
...
NVENCSTATUS CNvHWEncoder::NvEncCreateInputBuffer(uint32_t width, uint32_t height, void** inputBuffer, NV_ENC_BUFFER_FORMAT inputFormat) {
  ...
  if (nvStatus != NV_ENC_SUCCESS) {
    DEBUG_ERROR_VAR("NVIDIA encoding driver assert in NvEncCreateInputBuffer: ", to_string(nvStatus));
    assert(0);
  }
...
NVENCSTATUS CNvHWEncoder::NvEncDestroyInputBuffer(NV_ENC_INPUT_PTR inputBuffer) {
  ...
  if (inputBuffer) {
    ...
    if (nvStatus != NV_ENC_SUCCESS) {
      DEBUG_ERROR_VAR("NVIDIA encoding driver assert in NvEncDestroyInputBuffer: ", to_string(nvStatus));
      assert(0);
    }
  }
...
NVENCSTATUS CNvHWEncoder::NvEncCreateMVBuffer(uint32_t size, void** bitstreamBuffer) {
  ...
  if (status != NV_ENC_SUCCESS) {
    DEBUG_ERROR_VAR("NVIDIA encoding driver assert in NvEncCreateMVBuffer: ", to_string(status));
    assert(0);
  }
...
NVENCSTATUS CNvHWEncoder::NvEncDestroyMVBuffer(NV_ENC_OUTPUT_PTR bitstreamBuffer) {
  ...
  if (status != NV_ENC_SUCCESS) {
    DEBUG_ERROR_VAR("NVIDIA encoding driver assert in NvEncDestroyMVBuffer: ", to_string(status));
    assert(0);
  }
...
NVENCSTATUS CNvHWEncoder::NvEncCreateBitstreamBuffer(uint32_t size, void** bitstreamBuffer) {
  ...
  createBitstreamBufferParams.version = NV_ENC_CREATE_INPUT_BUFFER_VER;
  ...
  if (nvStatus != NV_ENC_SUCCESS) {
    DEBUG_ERROR_VAR("NVIDIA encoding driver assert in NvEncCreateBitstreamBuffer: ", to_string(nvStatus));
    assert(0);
  }
...
NVENCSTATUS CNvHWEncoder::NvEncDestroyBitstreamBuffer(NV_ENC_OUTPUT_PTR bitstreamBuffer) {
  ...
  if (bitstreamBuffer) {
    ...
    if (nvStatus != NV_ENC_SUCCESS) {
      DEBUG_ERROR_VAR("NVIDIA encoding driver assert in NvEncDestroyBitstreamBuffer: ", to_string(nvStatus));
      assert(0);
    }
  }
...
NVENCSTATUS CNvHWEncoder::NvEncLockInputBuffer(void* inputBuffer, void** bufferDataPtr, uint32_t* pitch) {
  ...
  if (nvStatus != NV_ENC_SUCCESS) {
    DEBUG_ERROR_VAR("NVIDIA encoding driver assert in NvEncLockInputBuffer: ", to_string(nvStatus));
    assert(0);
  }
...
NVENCSTATUS CNvHWEncoder::NvEncUnlockInputBuffer(NV_ENC_INPUT_PTR inputBuffer) {
  ...
  if (nvStatus != NV_ENC_SUCCESS) {
    DEBUG_ERROR_VAR("NVIDIA encoding driver assert in NvEncUnlockInputBuffer: ", to_string(nvStatus));
    assert(0);
  }
...
NVENCSTATUS CNvHWEncoder::NvEncDestroyEncoder() {
  ...
  if (m_bEncoderInitialized && m_pEncodeAPI) {
...
NVENCSTATUS CNvHWEncoder::NvEncOpenEncodeSessionEx(void* device, NV_ENC_DEVICE_TYPE deviceType) {
  ...
  if (nvStatus != NV_ENC_SUCCESS) {
    ...
    DEBUG_ERROR_VAR("NVIDIA encoding driver assert in NvEncOpenEncodeSessionEx.", to_string(nvStatus));
  }
...
#if defined(NV_WINDOWS)
#if defined (_WIN64)
	if (m_hinstLib == nullptr) m_hinstLib = LoadLibrary(TEXT("nvEncodeAPI64.dll"));
#else
	if (m_hinstLib == nullptr) m_hinstLib = LoadLibrary(TEXT("nvEncodeAPI.dll"));
#endif
#else
	if (m_hinstLib == nullptr) m_hinstLib = dlopen("libnvidia-encode.so.1", RTLD_LAZY);
#endif
...
NVENCSTATUS CNvHWEncoder::NvEncEncodeFrame(EncodeBuffer *pEncodeBuffer, NvEncPictureCommand *encPicCommand,
  ...
  if (nvStatus != NV_ENC_SUCCESS && nvStatus != NV_ENC_ERR_NEED_MORE_INPUT) {
    DEBUG_ERROR_VAR("NVIDIA encoding driver assert in NvEncEncodeFrame", to_string(nvStatus));    
    return nvStatus;
  }
...
NVENCSTATUS CNvHWEncoder::NvEncFlushEncoderQueue(void *hEOSEvent) {
  ...
  nvStatus = m_pEncodeAPI->nvEncEncodePicture(m_hEncoder, &encPicParams);
  if (nvStatus != NV_ENC_SUCCESS) {
    DEBUG_ERROR_VAR("NVIDIA encoding driver assert in NvEncFlushEncoderQueue", to_string(nvStatus));
    assert(0);
  }
  return nvStatus;
}
